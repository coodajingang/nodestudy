if (typeof (Worker) !== "undefined") {
  var ht = {
    arrowTo: function (canId, ox, oy, x1, y1, x2, y2) {
      ...
      ang = Math.atan(ang);
      if (end[1] - sta[1] >= 0) ctx.rotate(-ang);
      else ctx.rotate(Math.PI - ang);
      ctx.lineTo(-5, -10);
      ctx.lineTo(0, -5);
      ctx.lineTo(5, -10);
      ctx.lineTo(0, 0);
  ...
  var tu1 = {
    ...
    DiTu: function (S_x, S_y, S_r, S_f, M_x, M_y, M_r, M_f, E_x, E_y, E_r, E_f, e_x, e_y, e_r, e_f, s_x, s_y, s_f, m_x, m_y, m_f, Jb_xy, Jb_f) {
      ...
      if (J > 8 || J < 0) {
        ...
        if (el == "moon") m_f = "none";
        return
      } if (el == "sun") s_x = this.x0 + J * this.dx;
      s_y = this.y0 - W * this.dy;
      s_f = "block";
      if (el == "moon") m_x = this.x0 + J * this.dx;
  ...
  var dbj = "block", jx_xz = "\u9ec4\u8d64\u9053", jx_mc = "\u4ea4\u7ebf", bj_x = 0, bj_y = 0, dbj_fillcolor = "#0000FF";

  var tu2 = {
    isInit: 0, eR: 120, x0: 240, y0: 240, jxt: function (jx_mc, bj_x, bj_y, dbj_fillcolor) {
      ...
      if (hd) {
        ...
        if (a.I[1] < Math.PI / 2) dbj_fillcolor = "#00a0FF";
        else dbj_fillcolor = "#000000";
        dbj = "block"
      } else {
        jx_xz = "\u9ec4\u9053";
        jx_mc = "\u8d1d\u9ec4\u4ea4\u7ebf";
  ...
  var tu3 = {
    isInit: 0, eR: 250, x0: 250, y0: 250, eth_display: "block", Diqiu: function (dqcontext, Ex, Ey, Er) {
      Er = Cp10_eR.value - 0;
      ...
      for (var i = 0;
        ...
        if (c) dqcontext.moveTo(x, y);
        else dqcontext.lineTo(x, y)
      } dqcontext.stroke()
    }, showDitu: function () {
      ...
      for (k = 0;
        ...
        k++) {
          for (i = 0, f = 0;
            i <= N;
            i++)a[2 * i] = i * pi2 / N, a[2 * i + 1] = (k + 1) * pi / (m + 1) - pi_2;
        this.lineArr(a)
      } for (k = 0;
        k < n;
        k++) {
          for (i = 0, f = 0;
            i <= N;
            i++)a[2 * i] = k * pi2 / n, a[2 * i + 1] = i * pi / N - pi_2;
        this.lineArr(a)
      }
      ...
      this.eR = eR;
      this.F_ = F, this.J0_ = J0, this.W0_ = W0, this.eR_ = eR, this.jb_ = jb, this.tylx_ = tylx;
      touY.setlx(tylx, J0, W0, jb);

      var n;
      if (tylx == 3 || (tylx == 4 || (tylx == 5 || (tylx == 6 || (jb[2] < 1 || jb[3] < 1))))) this.eth_display = "none";
      else this.eth_display = "block";
      this.showDitu();
      this.drawJWQ(24, 11);
  ...
  var ht_b = {
    Vel: function (s) { return document.createElement("\x3cv:" + s + "/\x3e") }, INSel: function (el, el2) {
      el.insertBefore(el2);
      el2.style.zIndex--;
      el2.style.zIndex++
      ...
      if (name == "txt") s = 'textBox  inset\x3d"5px,5px,5px,5px" style\x3d"font-size:12px"';
      if (name == "sh") s = 'shadow on\x3d"T" type\x3d"single" color\x3d"#b3b3b3" offset\x3d"5px,5px"';
      if (name == "ext") s = 'extrusion on\x3d"True" color\x3d"red" rotationangle\x3d"0,0"';
      if (name == "fill") s = "fill";

      var el2 = ht_b.Vel(s);
      ht_b.INSel(el, el2, name);
      return el2
    }, ABSel: function (s) { return this.Vel(s + ' style\x3d"position:absolute"') }, Vel2: function (s) {
      var el = this.Vel(s + ' style\x3d"position:absolute"');
      el.zindex = function (index) { this.style.zIndex = index };
      ...
      var obj = new Object;
      for (var ns in ob) if (typeof ob[ns] == "object") obj[ns] = ht.clone(ob[ns]);
      else obj[ns] = ob[ns];
      return obj
    }
  };
  ...
  var ht = {
    canvas: "", qb: { fillcolor: "#FF0000", filled: "true", stroked: "true", strokecolor: "#00cc00", strokeweight: "1px", strokestyle: "solid" }, line: function (x1, y1, x2, y2) {
      ...
      el.p_cls = function () { this.phs.length = 0 };
      el.p_moveto = function (x, y) {
        var c = this.phs, n = c.length;
        c[n++] = "\x3dm\x3d", c[n++] = Math.round(x), c[n++] = Math.round(y)
      };
      el.p_lineto = function (x, y) {
        var c = this.phs, n = c.length;
        c[n++] = "\x3dl\x3d", c[n++] = Math.round(x), c[n++] = Math.round(y)
      };
      el.p_oval = function (x, y, sx, sy) { this.p_arc(x, y, sx, sy, 0, 360) };
      ...
      return this.shape(s)
    }, cls: function () { this.canvas.innerHTML = "" }, group: function (x, y, canvas) {
      ...
      ht_b.INSel(this.canvas, el);
      return el
    }, rect: function (x, y, sx, sy) { return this.rect0(x, y, sx, sy, "rect") }, oval: function (x, y, sx, sy) { return this.rect0(x, y, sx, sy, "oval") }, img: function (x, y, sx, sy, src) { return this.rect0(x, y, sx, sy, 'image src\x3d"' + src + '"') }, rrect: function (x, y, sx, sy, arc) { return this.rect0(x, y, sx, sy, "roundrect arcsize\x3d" + arc) }, arc: function (x, y, sx, sy, a, b) { return this.rect0(x, y, sx, sy, 'arc startAngle\x3d"' + a + '" endAngle\x3d"' + b + '"') }, point: function (x, y, s) {
  ...
  var tu1 = {
    isInit: 0, x0: 0, y0: 0, w: 0, h: 0, dx: 0, dy: 0, diming: new Array(116.4, 40, "\u5317\u4eac", 131.15, 46.63, "\u53cc\u9e2d\u5c71", 126, 46, "", 119, 25.4, "\u8386\u7530", 91, 29.7, "\u62c9\u8428", -73, 41, "\u7ebd\u7ea6", 151, -34, "\u6089\u5c3c", 37, 56, "\u83ab\u65af\u79d1", 30, -20, "\u975e\u6d32\u5357\u90e8", -56, -33, "\u4e4c\u62c9\u572d"), init: function (can) {
      ht.canvas = can;
      if (this.isInit) return;
      this.isInit = 1;
      this.w = String(can.style.width).replace(/[^0-9]/g, "");
  ...
  var tu2 = {
    isInit: 0, eR: 120, x0: 240, y0: 240, init: function (can) {
      ...
      if (!hd) {
        ...
        if (a.I[1] < Math.PI / 2) this.dbj.fillcolor = "#00a0FF";
        else this.dbj.fillcolor = "#000000";
  ...
  var tu3 = {
    isInit: 0, eR: 250, x0: 250, y0: 250, init: function (can) {
      ht.canvas = can;
      if (this.isInit) return;
      this.isInit = 1;
      this.w = String(can.style.width).replace(/[^0-9]/g, "");
